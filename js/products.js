...
/* ─────────────────────────────────────────────
   SHEET CONFIG
   Spreadsheet ID: 1rvpPTcihhq2dEW8fdZhbKuNsOUrsnIk0cTkjdYCJ5Eg
   The SHEET_TAB below should match the tab name in your Google Sheet
   (e.g. "Sheet1", "Products", "Catalogue" — check the tab at the bottom of your sheet)
   ─────────────────────────────────────────────*/
const SPREADSHEET_ID  = '1rvpPTcihhq2dEW8fdZhbKuNsOUrsnIk0cTkjdYCJ5Eg';
const SHEET_TAB       = 'Products'; // ← Change this if your tab is named differently
...
async function fetchFromGoogleSheet() {
  ...
  try {
    console.log('[Sajkriiti] Fetching products from Google Sheet\u2026', GOOGLE_SHEET_URL);
    const res = await fetch(GOOGLE_SHEET_URL);

    if (!res.ok) {
      const errText = await res.text().catch(() => '');
      console.error('[Sajkriiti] Sheet fetch failed (HTTP ' + res.status + '):', errText);
      if (res.status === 404) {
        console.error(
          '[Sajkriiti] 404 \u2014 Sheet tab not found. ' +
          'Make sure SHEET_TAB ("' + SHEET_TAB + '") exactly matches a tab name in your Google Sheet. ' +
          'Also ensure the sheet is published: File \u2192 Share \u2192 Publish to web.'
        );
      }
    ...
    if (!Array.isArray(data) || data.length === 0) {
      console.warn('[Sajkriiti] Sheet returned empty data \u2014 showing fallback products.');
      return null;
    }

    console.log('[Sajkriiti] Loaded ' + data.length + ' products from Google Sheet \u2713');

    // Normalise: accept any sensible column-name casing
    const get = function(row) {
      ...
      for (var ki = 0; ki < keys.length; ki++) {
        ...
        for (var ri = 0; ri < rowKeys.length; ri++) {
          if (rowKeys[ri].toLowerCase() === k.toLowerCase() && row[rowKeys[ri]] !== undefined && row[rowKeys[ri]] !== null && row[rowKeys[ri]] !== '') {
    ...
    return data.map(function(row, i) {
      return {
        id:           get(row, 'id', 'ID', 'Sr', 'sr_no', 'Serial') || (i + 1),
        name:         get(row, 'name', 'Name', 'Product', 'product_name', 'Title'),
        desc:         get(row, 'desc', 'description', 'Description', 'Details', 'details'),
        price:        get(row, 'price', 'Price', 'MRP', 'Rate', 'Amount'),
        category:     (get(row, 'category', 'Category', 'Type', 'type') || 'bouquets').toLowerCase().trim(),
        image:        get(row, 'image', 'Image', 'image_url', 'Image URL', 'img', 'photo', 'Photo'),
        badge:        get(row, 'badge', 'Badge', 'Tag', 'Label'),
        whatsapp_msg: get(row, 'whatsapp_msg', 'whatsapp', 'WhatsApp', 'wa_msg', 'custom_msg'),
      };
    }).filter(function(p) { return String(p.name).trim() !== ''; });

  } catch (e) {
    console.warn('[Sajkriiti] Google Sheet fetch failed \u2014 using fallback products.', e.message);
    return null;
  }
}
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (sheetData && sheetData.length > 0) {
    ...
    if (loader) loader.style.display = 'none';
    // Show a subtle dev-only warning in the console
    console.info(
      '%c[Sajkriiti] Showing fallback products. ' +
      'To display live products, make sure your Google Sheet is published and SHEET_TAB matches your tab name.',
      'color: #c0576e; font-weight: bold;'
    );
  }

  // Hide the dev notice if sheet is connected successfully
